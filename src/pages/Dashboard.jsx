...
export default function Dashboard() {
  ...
  const [isSendingRiderReport, setIsSendingRiderReport] = useState(false);
  ...
  const handleSendRiderReports = async () => {
    if (!window.confirm('Send personalized WhatsApp status to all riders for today?')) return;
    setIsSendingRiderReport(true);
    try {
      const resp = await fetch('/api/rider-report', { method: 'POST' });
      const data = await resp.json();
      if (data.success) {
        toast.success(`✅ Sent to ${data.sent} rider(s). ${data.skipped ? `${data.skipped} skipped (no number).` : ''}`);
      } else {
        toast.error(data.error || 'Failed to send rider reports.');
      }
    } catch (err) {
      toast.error('Network error. Check if the server is running.');
    } finally {
      setIsSendingRiderReport(false);
    }
  };
    ...
    <div className="space-y-6">
      ...
      >
        ...
        <div className="flex items-center gap-3">
           ...
           <button
            onClick={handleSendRiderReports}
            disabled={isSendingRiderReport}
            title="Send today's personalized status to each rider via WhatsApp"
            className="flex items-center gap-2 px-4 py-2 rounded-xl bg-green-600 hover:bg-green-700 disabled:opacity-60 text-white text-xs font-bold shadow-lg shadow-green-600/20 transition-all active:scale-95"
          >
            {isSendingRiderReport ? (
              <><span className="w-3.5 h-3.5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> Sending...</>
            ) : (
              <><MessageSquare className="w-3.5 h-3.5" /> Send Rider Reports</>
            )}
          </button>
        </div>
      </motion.div>
