...
import { IndianRupee, Lock, Calendar, Edit2, Share2, Users, FileText, CheckCircle2 } from 'lucide-react';
...
export default function Earnings() {
  ...
  const dailyEarnings = useMemo(() => {
    ...
    const completedRiders = record?.riders || [];
    
    // Map across all riders to ensure everyone shows up, or just those who worked?
    // Let's map across all active riders to support manual override even if they didn't deliver.
    return riders.map(r => {
      // Find what they actually did today
      const deliveryData = completedRiders.find(dr => dr.riderId === r.id);
      const totalDelivered = deliveryData ? (Number(deliveryData.completedDelivery) || 0) + (Number(deliveryData.completedPickup) || 0) : 0;
      
      const autoPresent = totalDelivered > 0;
      const autoSalary = totalDelivered * 12;

      // Check for OVERRIDES in earningsService
  ...
  }, [activeDate, dailyRecords, riders, overrides]);
  ...
  const monthlyEarnings = useMemo(() => {
    ...
    // Create a map per rider
    const summary = {};
    riders.forEach(r => summary[r.id] = { riderName: r.name, totalDays: 0, totalDeliveries: 0, totalSalary: 0 });

    // Loop through all days in this activeMonth
    ...
    for (let day = 1; day <= daysInMonth; day++) {
       ...
       const compRiders = dailyRecord?.riders || [];
       
       riders.forEach(r => {
          const mRider = compRiders.find(dr => dr.riderId === r.id);
          const tDels = mRider ? (Number(mRider.completedDelivery) || 0) + (Number(mRider.completedPickup) || 0) : 0;
  ...
  }, [activeMonth, dailyRecords, riders, overrides, activeTab]);
             ...
             <table className="w-full text-sm text-left">
               <thead className="bg-gray-50 dark:bg-gray-800 text-gray-500 font-bold uppercase tracking-wider text-[11px]">
                 <tr>
                   ...
                   <th className="px-6 py-4 text-center">Salary (₹12/px)</th>
                 ...
                 {dailyEarnings.map(r => (
                   <tr key={r.riderId} className="hover:bg-gray-50/50 dark:hover:bg-gray-800/30">
                     <td className="px-6 py-4 font-bold dark:text-white flex items-center gap-2">
                       {r.riderName} {r.isCustom && <span className="text-[10px] bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded uppercase font-bold tracking-widest">Edited</span>}
                     </td>
